...
let BRAND = {
  ...
  primaryColor: '#7eb8f7',
  sidebarColor: '#111118',
  navActiveColor: 'rgba(126,184,247,.15)',
  fontFamily: 'Poppins',
  borderRadius: '12px',
...
function setLoading(on, msg='') {
  ...
  if (!ov) {
    ...
    ov.innerHTML = `<div style="width:36px;height:36px;border:3px solid rgba(126,184,247,.3);border-top-color:var(--gold);border-radius:50%;animation:spin 0.8s linear infinite"></div><div id="loading-msg" style="font-size:.875rem;color:rgba(255,255,255,.7)"></div>`;
...
function renderDash() {
  ...
  if(DB_ANNOUNCEMENTS.length>0) {
    ...
    banner.style.cssText='background:linear-gradient(135deg,#1a2a0f,#2a3a1a);border:1px solid rgba(126,184,247,.3);border-radius:10px;padding:1rem 1.25rem;margin-bottom:1.25rem;display:flex;align-items:center;gap:.875rem;color:#fff';
...
function renderProfile() {
  ...
  allC().forEach(c=>{
    ...
    const color = ov.color || c.color || '#7eb8f7';
...
function sclr(s) { const c=['#7eb8f7','#2c3e50','#8B4513','#1a3a5c','#4a1a5c','#1a5c3a','#8e44ad']; let h=0; for(let i=0;i<s.length;i++) h=s.charCodeAt(i)+((h<<5)-h); return c[Math.abs(h)%c.length]; }
...
async function renderAC(tab) {
  ...
  // ── BADGES ──
    ...
    courses.forEach(course => {
      ...
      const color = ov.color || course.color || '#7eb8f7';
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.25rem">
              <div class="form-group" style="margin:0">
                ...
                <div style="display:flex;gap:.6rem;align-items:center">
                  ...
                  <input type="text" id="b-color-hex" value="${BRAND.primaryColor}" style="width:100px;font-family:monospace" oninput="if(this.value.match(/^#[0-9a-fA-F]{6}$/)){document.getElementById('b-color').value=this.value;bPreview()}" placeholder="#7eb8f7">
                </div>
              </div>
              <div class="form-group" style="margin:0">
  ...
  courses.forEach(course => {
    ...
    if (emoji || color || label) {
      BADGE_OVERRIDES[course.id] = {
        ...
        color: color || course.color || '#7eb8f7',
